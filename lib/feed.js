...
function Feed(opts) {
  ...
  self.feed = opts.feed || 'continuous';
  ...
  self.seq_interval = opts.seq_interval || DEFAULT_SEQ_INTERVAL;
...
Feed.prototype.follow = function follow_feed() {
  ...
  if (!self.db) { throw new Error('Database URL required'); }

  if (['continuous', 'longpoll', 'normal'].indexOf(self.feed) === -1) {
    throw new Error('Unsupported feed type: ' + self.feed);
  }

  if (self.db.match(/\/_db_updates$/)) { self.is_db_updates = true; }

  if (self.is_db_updates) { delete self.since; }

  if (typeof self.heartbeat !== 'number') { throw new Error('Required "heartbeat" value'); }
...
Feed.prototype.confirm = function confirm_feed() {
  ...
  function db_response(er, resp, body) {
    ...
    if (!self.is_db_updates && !self.dead && (!db.db_name || !db.instance_start_time)) { return self.emit('error', new Error('Bad DB response: ' + body)); }

    if (self.is_db_updates && !self.dead && !db.couchdb) { return self.emit('error', new Error('Bad server response: ' + body)); }

    if (!self.is_db_updates) { self.original_db_seq = db.update_seq; }
    ...
    if (self.since === -1) {
      ...
      if (isNaN(db.update_seq)) { return self.emit('error', new Error('DB requires specific id in "since"')); }
...
Feed.prototype.query = function query_feed() {
  ...
  FEED_PARAMETERS.forEach(function(key) {
    if (key in self) { query_params[key] = self[key]; }
  });

  if (typeof query_params.filter !== 'string') { delete query_params.filter; }
  ...
  var poll_size = 100;
  if (query_params.feed === 'longpoll' && (!query_params.limit || query_params.limit > poll_size)) { query_params.limit = poll_size; }

  var feed_url = self.db + (self.is_db_updates ? '' : '/_changes');
  ...
  function on_feed_response(er, resp, body) {
    ...
    if ((resp !== undefined && resp.body) || body) { return self.die(new Error('Cannot handle a body in the feed response: ' + lib.JS(resp.body || body))); }
...
Feed.prototype.prep = function prep_request(changes_stream) {
  ...
  if (self.is_paused) { self.pause(); } else { self.resume(); }
  ...
  function handler_for(ev) {
    ...
    function handle_confirmed_req_event() {
      if (self.pending.request === changes_stream) { return inner_handler.apply(self, arguments); }

      if (!changes_stream.created_at) { return self.die(new Error('Received data from unknown request')); } // Pretty sure this is impossible.

      var s_to_now = (new Date() - changes_stream.created_at) / 1000;
      var s_to_req = '[no req]';
      if (self.pending.request) { s_to_req = (self.pending.request.created_at - changes_stream.created_at) / 1000; }
...
Feed.prototype.wait = function wait_for_event() {
  ...
  if (self.pending.wait_timer) { return self.die(new Error('wait() called but there is already a wait_timer: ' + self.pending.wait_timer)); }
  ...
  if (self.inactivity_ms) { msg += ', inactivity=' + self.inactivity_ms; }
  msg += ': ' + self.db_safe;
...
Feed.prototype.pause = function() {
  ...
  self.is_paused = true;
  if (self.pending && self.pending.request && self.pending.request.pause) { self.pending.request.pause(); } else { self.log.warn('No pending request to pause'); }

  if (!was_paused) { self.emit('pause'); }
};
...
Feed.prototype.resume = function() {
  ...
  self.is_paused = false;
  if (was_paused) { self.emit('resume'); }

  if (self.pending && self.pending.request && self.pending.request.resume) { self.pending.request.resume(); } else { self.log.warn('No pending request to resume'); }
};
...
Feed.prototype.on_couch_heartbeat = function on_couch_heartbeat() {
  ...
  self.got_activity();
  if (self.dead) { return self.log.debug('Skip heartbeat processing for dead feed'); }

  self.emit('heartbeat');

  if (self.dead) { return self.log.debug('No wait: heartbeat listener stopped this feed'); }
  self.wait();
};
...
Feed.prototype.on_couch_data = function on_couch_data(change) {
  ...
  self.got_activity();
  if (self.dead) { return self.log.debug('Skip data processing for dead feed'); }
  ...
  if ('last_seq' in change) {
    if (self.feed === 'normal' && (change.pending === undefined || change.pending === 0)) {
      self.seen_last_seq = true;
      self.emit('last_seq', change.last_seq);
      return self.stop();
    } else {
  ...
  if (!self.is_db_updates && self.seq_interval === 1 && !change.seq) {
  ...
  if (self.dead) { return self.log.debug('No wait: change listener stopped this feed'); }
  self.wait();
};

Feed.prototype.on_timeout = function on_timeout() {
  var self = this;
  if (self.dead) { return self.log.debug('No timeout: change listener stopped this feed'); }
...
Feed.prototype.retry = function retry() {
  ...
  self.retry_delay *= 2;
  if (self.retry_delay > max_retry_ms) { self.retry_delay = max_retry_ms; }
};
...
Feed.prototype.die = function(er) {
  ...
  if (er) { self.log.fatal('Fatal error: ' + er.stack); }
...
Feed.prototype.on_change = function on_change(change) {
  ...
  if (!self.is_db_updates && self.seq_interval === 1 && change.seq === self.since) {
    self.log.debug('Bad seq value ' + change.seq + ' since=' + self.since);
    return destroy_req(self.pending.request);
  }

  if (typeof self.filter !== 'function') { return self.on_good_change(change); }

  var req = lib.JDUP({'query': self.pending.request.changes_query});
  var filter_args;

  if (self.is_db_updates && change.seq) {
    if (!change.db_name || !change.type) { return self.die(new Error('Internal _db_updates filter needs .db_name and .type in change ', change)); }
    filter_args = [change.db_name, change.type, req];
  } else {
    if (!change.doc) { return self.die(new Error('Internal filter needs .doc in change ' + change.seq)); }
...
Feed.prototype.on_good_change = function on_good_change(change) {
  ...
  if (self.inactivity_ms && !self.inactivity_timer) { return self.die(new Error('Cannot find inactivity timer during change')); }

  clearTimeout(self.inactivity_timer);
  self.inactivity_timer = null;
  if (self.inactivity_ms) { self.inactivity_timer = setTimeout(function() { self.on_inactivity(); }, self.inactivity_ms); }

  self.change_at = new Date();

  if (!self.is_db_updates) { self.since = change.seq; }
...
function destroy_req(req) {
  if (req) { destroy_response(req.response); }
...
function destroy_response(response) {
  ...
  if (response.connection) { response.connection.destroy(); }
}
