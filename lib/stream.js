...
function Changes(opts) {
  ...
  self.on('pipe', function(src) {
    if (!self.source) { self.source = src; } else {
...
Changes.prototype.pause = function() {
  ...
  if (self.source && self.source.pause) { self.source.pause(); }
};

Changes.prototype.resume = function() {
  var self = this;
  self.is_sending = true;
  if (self.source && self.source.resume) { self.source.resume(); }
  self.emit_changes();
};
...
Changes.prototype.write = function(data, encoding) {
  ...
  if (typeof data !== 'string') { return; } // Looks like normalize_data emitted an error.
...
Changes.prototype.write_continuous = function(data) {
  ...
  while ((offset = buf.indexOf('\n')) >= 0) {
    ...
    else if (json[0] !== '{') { return self.error(new Error('Non-object JSON data: ' + json)); } else {
      try { change = JSON.parse(json); } catch (er) { return self.error(er); }
...
Changes.prototype.write_normal = function(data) {
  var self = this;
  ...
  while ((offset = buf.indexOf('\n')) >= 0) {
    ...
    if (json.match('^"last_seq":') !== -1) {
      // make 'last_seq' a valid JSON change
      json = json.replace('"last_seq":', '{"last_seq":');
    }
  ...
  // Remember the unused data and send all known good changes. The data event listeners may call
  // .pause() so remember the is_sending state now before calling them.
  var wasSending = self.is_sending;
  self.buf = buf;
  self.emit_changes();
  return wasSending;
};
...
Changes.prototype.end = function(data, encoding) {
  ...
  if (self.feed === 'longpoll') {
    ...
    try { changes = JSON.parse(changes) || {}; } catch (er) { return self.error(er); }

    if (!Array.isArray(changes.results)) { return self.error(new Error('No "results" field in feed')); }
    if (self.changes.length !== 0) { return self.error(new Error('Changes are already queued: ' + JSON.stringify(self.changes))); }
    ...
    if (self.buf !== '') { self.log.debug('Unprocessed data after "end" called: ' + util.inspect(self.buf)); }
  }
};
...
Changes.prototype.destroy = function() {
  ...
  if (self.source && typeof self.source.abort === 'function') { return self.source.abort(); }

  if (self.source && typeof self.source.destroy === 'function') { self.source.destroy(); }
  ...
  typeof self.source.response.destroy === 'function') { self.source.response.destroy(); }
};
...
Changes.prototype.normalize_data = function(data, encoding) {
  ...
  if (data instanceof Buffer) { data = data.toString(encoding); } else if (typeof data === 'undefined' && typeof encoding === 'undefined') { data = ''; }

  if (typeof data !== 'string') { return self.error(new Error('Not a string or Buffer: ' + util.inspect(data))); }
  ...
  if (prefix !== expected_part) { return self.error(new Error('Prefix not expected ' + util.inspect(expected_part) + ': ' + util.inspect(prefix))); }
